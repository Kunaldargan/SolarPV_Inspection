...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        cout << "Usage gammaCorrection <inputDiretory> <outputDirectory>" << endl;
        return -1;
    }

    string im, imgPath, input = argv[1], outputDir = argv[2];
    vector<string> images;

    DIR *directory = opendir(input.c_str());  // read directory
    struct dirent *dp;

    while ((dp = readdir(directory)) != NULL) {
        if (!std::strcmp(dp->d_name, ".") || !std::strcmp(dp->d_name, "..")) {  // check for files
            continue;
        }
        im = dp->d_name;
        images.push_back(im);  // All images
    }
    ...
    for (auto i : images) {
        ...
        cout << mean_pixel(img) << endl;

        Mat out;

        cout << "Denoising ..." << endl;
        fastNlMeansDenoisingColored(img, out);
        imwrite(outputDir + "/denoised_" + i, out);

        cout << "White balancing ..." << endl;
        out = white_balance(out.clone());
        imwrite(outputDir + "/white_balanced_" + i, out);
        ...
        imwrite(outputDir + "/gamma_corrected_" + i, out);
    }
